/* This file, `src/env.js`, is automatically generated by `create-t3-app`. The core
...
//
// Other common chains you'll see are `.min(1)` (must not be an empty string) or `.optional()`
...
// This line creates and exports the main `env` object that the rest of your application will import and use.
...
export const env = createEnv({
  ...
  server: {
    ...
    // `AUTH_SECRET`: This key must exactly match the variable name in your `.env` file.
    AUTH_SECRET:
      ...
      process.env.NODE_ENV === "production"
        ...
        // NOTE: By itself, `z.string()` does allow an empty string (`""`). To prevent this, you would
        // need to chain `.min(1)` (e.g., `z.string().min(1)`). In this case, an empty string is likely
        // considered an invalid secret and would cause an error later, but the schema here only checks the type.
        ? z.string()
    ...
    // This defines the validation rule for the `NODE_ENV` environment variable itself.
    // It's a best practice to validate this crucial variable to ensure it's one of the expected values.
    NODE_ENV: z
  ...
  //
  // **What is `process.env`? (And the Next.js / Node.js relationship)**
  //
  // 1. **Node.js is the Server Environment:** Node.js is a JavaScript "runtime environment". It's a program
  //    that allows you to execute JavaScript code *outside* of a web browser, for example, on a server.
  //    In any Node.js application, `process` is a global object that provides information about the
  //    current running process.
  //
  // 2. **Next.js Runs on Node.js:** Your Next.js application is, at its core, a sophisticated Node.js
  //    application. When you run `npm run dev` or `npm start`, you are starting a Node.js server that is
  //    running the Next.js framework. All of your "server-side" code (in `src/server`, API routes, and
  //    Server Components) is executed by this Node.js process.
  //
  // 3. **The `process.env` Object:** Because your server code runs in Node.js, it has access to the
  ...
  //
  // 4. **The Client (Browser) is Different:** The user's web browser is a completely separate environment.
  //    It does **not** have a `process` object. The client-side JavaScript code that runs in the browser
  //    cannot directly access `process.env`.
  //
